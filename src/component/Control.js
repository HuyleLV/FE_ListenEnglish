...
function Control({
    ...
    const skipForward = (time) => {
        if (time === 'forward' && audioElem.current?.currentTime) {
            videoPlayerRef?.current?.seekTo(audioElem.current.currentTime + 10)
            ...
            <center>
                <div className='p-4 bg-gradient-to-r from-red-500 to-red-800 flex flex-row justify-center'>
                    <button type="button" className='mx-5 rounded-full p-4 bg-orange-600' onClick={changeRepeatMode}>
                        <img src={repeatMode === 'off' ? repeatOff : repeatMode === 'track' ? repeatTrack : repeatOne}
                             className='w-10 h-10'/>
                    </button>
                    <button type="button" className='mx-5 h-auto rounded-full px-9 bg-orange-600 items-center'
                    ...
                    <button type="button" className='mx-5 rounded-full p-4 bg-orange-600' onClick={changeSoundMode}>
                        <img
                            src={audioElem.current?.volume === 1 ? volumeHigh : audioElem.current?.volume === 0.25 ? volumeLow : audioElem.current?.volume === 0.5 ? volumeMedium : volumeMute}
                            className='w-10 h-10'/>
                    </button>
