    ...
    if(value > 0){
      return (
        <>
          ...
          <p className="flex font-semibold text-lg">Content: <Paragraph copyable className="pl-4 font-semibold text-lg">{cookies?.user?.email}</Paragraph></p>
          <p className="flex font-semibold text-lg">Price: <Paragraph copyable className="pl-4 font-semibold text-lg">{value}</Paragraph></p>
          
          <div className="flex justify-center py-5">
            <img src={`https://img.vietqr.io/image/MB-0336877888-compact.png?amount=${value}&addInfo="${cookies?.user?.email}"`} className="w-[400px]"/>
          </div>
        </>
      )
    }
